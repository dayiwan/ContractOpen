...
import { getMyResourceExamineDetailApi} from "@/api/MyResource.js"
export default {
    name: "MyResourceExamineDetail",
    ...
    methods: {
        ...
        async getMyResourceDetail(){
            const res = await getMyResourceExamineDetailApi(this.$route.query.id);
            const tabledata = res.list[0];
            this.ruleForm.name = tabledata.blockDataName;
            this.ruleForm.dateStart = tabledata.createTime.split("T")[0];
